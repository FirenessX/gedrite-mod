...
public class MetalDetectorItem extends Item{
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        user.getEntityWorld().playSound(user, user.getBlockPos(), ModSoundEvents.METAL_DETECTOR_USE, SoundCategory.PLAYERS);
        if(world.isClient()){
        ...
        user.getMainHandStack().damage(1, user, hand.getEquipmentSlot());
    ...
    private void outputValuableCoordinates(BlockPos blockPos, PlayerEntity player, Block block) {
        ...
        if (player.getEntityWorld().isClient()) {
            foundPos = blockPos;
            foundState = player.getEntityWorld().getBlockState(blockPos);
        }
    }
